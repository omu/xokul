...
module Yoksis
  module Resumes
    class LecturesSerializer < Serializer
      attribute(:id)                           { object[:ders_id].safe_to_i                       }
      attribute(:name)                         { object[:ders_adi].titleize_tr                    }
      ...
      attribute(:period)                       { object[:ders_saati].safe_to_i                    }
      ...
      attribute(:date_of_insert)               { Time.zone.parse object[:ekleme_tarihi] || ''     }
    end
  end
end
