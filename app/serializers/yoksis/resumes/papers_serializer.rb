...
module Yoksis
  module Resumes
    class PapersSerializer < Serializer
      ...
      attribute(:authors)                { object[:yazar_adi].split(',').map(&:strip).map(&:titleize_tr) }
      ...
      attribute(:status_id)              { object[:yayin_durumu].safe_to_i                               }
      attribute(:status_name)            { object[:yayin_durumu_ad].titleize_tr                          }
